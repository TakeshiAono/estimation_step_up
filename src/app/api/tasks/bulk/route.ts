...
const extractHierarchy = (parentTask: BigTask): Hierarchy[] => {
  ...
  return childrenTaskId.map((childTaskId: number) => {
    return { parentId: parentId, childId: childTaskId };
  });
};
...
const depthSearch = (parentTask: BigTask): Hierarchy[] => {
  ...
  const taskInfo: Hierarchy[] = childrenTask.map((childTask) =>
    depthSearch(childTask),
  )[0]; // 1引数は定義せず 2引数を定義
...
export async function PUT(
  ...
  try {
    ...
    const hierarchyList: Hierarchy[] = _.flatten(
      tasks.map((topTask) => depthSearch(topTask)),
    );
